...
void buildCandidateSet(unsigned int setIndexBits, addr_list_t *evSetCandidates,
    ...
    // make cursor start at first byte of cache lines
    cur = (uint8_t*) (((uintptr_t)(cur + cache->info.linesize))
            & ~(((uintptr_t) cache->info.linesize) - 1));

    unsigned desiredCandidateCount = cache->info.ways * cache->info.slices * 16;

    for (size_t i = 0;
            i < (shmSizeInByte - 2 * cache->info.linesize)
